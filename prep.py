"""Step 2 — load from HF, clean, split, re-upload train/test."""
import os
import pandas as pd
...
# Load raw data
ds = load_dataset(DATASET_REPO, data_files="data/tourism.csv", split="train")
df = ds.to_pandas()
...
for fname in ["train.csv", "test.csv"]:
    api.upload_file(
        ...
        repo_type="dataset",
        token=HF_TOKEN,
        commit_message=f"CI: upload {fname}",
    )
